...
const srcCompPath = path.join(rootPath, 'src/components'); // packages/components
const docPath = path.join(rootPath, 'docs/components');
...
function replaceInfo(filePath, componentName, componentDesc) {
    fs.readFile(filePath, (err, data) => {
        ...
        str = str
            ...
            .replace(/{{component-desc}}/g, componentDesc);
    ...
    filesPath.forEach(async ({ target }) => {
        walk(target, function (path, fileName) {
            ...
            const newPath =
                path +
                '/' +
                fileName.replace('component-template', componentName);
...
async function addCommponentInConfig({
    componentName,
    componentZhName,
    componentDesc,
}) {
...
async function modifyEntry({ componentName }) {
    ...
    lines.push(
        `export { default as ${_.startCase(componentName).replace(
            /\s/g,
            ''
...
async function updateConfig() {
    ...
    fs.writeFileSync(
        configPath,
        oldContent.replace(/业务组件库/, '$业务组件库$')
    );
    fs.writeFileSync(
        configPath,
        oldContent.replace(/\$业务组件库\$/, '业务组件库')
...
export async function addComponent(options) {
    ...
    const tasks = new Listr([
        ...
        },
        {
            title: 'add config',
            task: () =>
                addCommponentInConfig(options).catch((e) => console.warn(e)),
        },
        {
            title: 'Modify Entry',
