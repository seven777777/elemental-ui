...
async function addSourceFiles(project) {
    ...
    const filePaths = excludeFiles(
        await glob([globSourceFile, '!elemental-ui/**/*'], {
            cwd: pkgRoot, // 读取 packages 目录下除了 elemental-ui 目录的文件
            absolute: true, // 读取绝对路径
            onlyFiles: true, // 只读取文件
        })
    );
    const etPaths = excludeFiles(
        await glob(globSourceFile, {
            cwd: etRoot, // 读取 ./packages/elemental-ui 目录下的文件
            onlyFiles: true, // 只读取文件
        })
    );
    ...
    await Promise.all([
        ...
        ...etPaths.map(async (file) => {
            // 读取 ./packages/elemental-ui 目录下的文件，并手动通过 createSourceFile 方法添加 ts-morph 项目的 TypeScript 源文件
            const content = await readFile(path.resolve(etRoot, file), 'utf-8');
